...
5. (optional) **Cloudflare Email Service** — the only email path for now, used via the admin Worker's `send_email` binding to send invite/re-issue links. Sending to arbitrary recipients requires the **Workers Paid plan** and an **onboarded sending domain** (DNS records); the free allotment is 3,000 emails/month within that plan. **The `*.workers.dev` hostname cannot be that domain — Email Routing only attaches to a real zone you control (its DNS), so a no-custom-domain instance cannot send via Cloudflare; use copy-paste links.** Without email, invites are copy-paste links (free, no domain) — the one-click "Copy link" on the invite page covers the flow. Other providers (e.g. SendGrid single-sender, for no-domain automated email) are not implemented yet.
...
2. **Users list.** Email; last installed and last updated timestamps; current version; assigned streams; pinned version; status. A **no-build icon** on affected rows and a **filter for "no available build"** (§11); also filter by stream, status, and pinned. Because stream assignment is optional, a user with **no channel and no pin** is shown a **"no channel" warning** in the Channels column (they resolve to the §11 empty state — no updates until assigned), and the Add-user form carries the same hint. All list tables also sort by any column client-side (decision [0012](decisions/0012-admin-table-enhancer.md)).
...
12. **Upload build (browser).** Upload an already-signed, notarized archive and paste its EdDSA signature; set short/build version, min OS, critical, and stream assignment. Picking the archive **autofills** short/build version + min OS from the bundle's `Info.plist` (editable). Two modes share one form and endpoint: **Normal release**, and **Rollback** — which only adds §9 roll-forward guidance (rebuild old code with a higher build number; shows the current highest as the floor), since a rollback *is* a normal upload of a rebuilt artifact. A **no-channel hint** notes that a build linked to no channel reaches no one. The Worker stores the archive in R2 and registers the build row — it never signs (§20). Subject to the request-size limit; very large builds use the CI/script path.
