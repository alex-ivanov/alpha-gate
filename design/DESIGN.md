...
- **Tamper-evident audit (hash chain + anchor).** The `admin_audit` rows are **hash-chained** — each stores `prev_hash` and `hash = SHA-256(prev_hash ‖ canonical(entry))` — so edits and mid-chain deletions break verification. A chain alone can't catch *truncation* (deleting the newest rows) or a full *rebuild*, so the daily cron **anchors** the current chain head (latest `hash` + row count) where the running admin can't silently rewrite it: into the bucket-locked R2 object and an email to the owner. Detection becomes a simple check — does the live chain extend the last anchored head? If it's shorter, diverged, or reset, something was tampered with since the last anchor.
