...
Creating the user lands you on a page titled "Invite ready". It shows two blocks:
...
On a deployed workers.dev instance the link carries the public host. On local dev or a custom domain the admin cannot derive the public host, and the page warns that the shown link carries the admin host instead — a user cannot open that one.
...
The public host also serves a request page at `/access`: an email field and a *Request access* button. Anyone with the URL can ask for access; nothing is granted automatically.
...
- *Invite* creates the user and shows the same invite page as *Add user*. If the email belongs to an existing revoked user, Invite issues a fresh token and reactivates them; the old link stays dead.
...
Copy-paste is the default and needs no setup: copy the link or the filled message from the invite page and send it over whatever channel you already share with the user.
