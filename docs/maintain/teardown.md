...
```bash
./deploy/teardown.sh --instance <slug>               # archive the database, confirm, destroy
...
Teardown also does not touch a CI service token. If you created one for headless publishing, remove it under Zero Trust → Access → Service Auth.
...
Delete either from your login Keychain by hand if you want them gone.
